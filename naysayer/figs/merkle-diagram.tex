\begin{figure*}[tbh]
    ...
    \end{tikzpicture}
    \caption{Each node in a Merkle tree consists of a hash of its children. The root $h$ is a commitment to the vector of leaves $(v_0, v_1, \dots, v_7)$. An opening proof for the element $v_2$ is its copath (black nodes); the ``verification trace'' for the proof is the path (blue nodes).}
