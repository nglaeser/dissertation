...
\bigO{\polylog{\sizeof{x}}}$ (by the succinctness of $\Pi$). Furthermore, it is clear to see that the runtime of $\vrfynay$ is constant.
\end{proof}

The proof of \Cref{thm:naysayer} gives a generic way to build a succinct naysayer proof system for any succinct proof system $\Pi$. The generic construction even allows efficient naysaying, since the runtime of $\naysay$ depends only on the runtime of $\Pi.\vrfy$, which is sublinear due to the succinctness of $\Pi$.
