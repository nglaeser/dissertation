...
The proof of \Cref{thm:naysayer} gives a generic way to build a succinct naysayer proof system for any proof system $\Pi$ with polynomial-time verification. For strongly succinct\todo{define or change} proof systems, the generic construction even allows efficient naysaying, since the runtime of $\naysay$ depends only on the runtime of $\Pi.\vrfy$, which is sublinear if $\Pi$ is strongly succinct.
