...
(Note this does not include the size of $\aux$; we will see in the next section that this does not affect the performance of the verifier.)
Furthermore, in most cases, the naysayer proof consists of an \emph{integer} index or indices rather that group or field elements; representing the former requires only a few bits compared to the latter (which are normally at least $\secpar$ bits long). This means that, in practice, naysayer proofs can offer \emph{practically} smaller proofs sizes even when they are not asymptotically smaller, such as when the original proof is constant-size (e.g., a few group elements). \todo{Need to check this with some common circuit sizes (e.g., the STARK verification circuit) and group element sizes (e.g., BN254 curve G1 element or other).}