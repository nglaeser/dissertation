...
\begin{table}[tbh!]
   ...
   \makebox[\linewidth]{
    ...
    \toprule
     ...
     \midrule
     $\sizeof{\pi}$ & $\bigO{\secpar\log^2(d)}\mathbb{H}$\ & $\bigO{\secpar}\FF$\  & $\bigO{\secpar}\FF$\  & $\bigO{\sqrt{n}}\GG$\  \\ 
     $\vrfy$ & $\mathcal{O}(\secpar\log^2(d))\mathbb{H}$\ & $\mathcal{O}(\secpar)\FF+1\mathbb{H}$\  & $\mathcal{O}(\secpar)\mathbb{H}$\  & $\mathcal{O}(n)\GG$\  \\\midrule
