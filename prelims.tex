...
\todo{NP-languages and relations} \noemi{A language is some subset of strings. Every NP language has a corresponding polynomially-decidable relation (i.e., decidable by a circuit $C(x,w)$ such that $\sizeof{C} \in \bigO{\sizeof{x}}$) such that if $x \in \Lang, \exists w \suchthat\allowbreak (x,w) \in \Rel_\Lang$ where $w$ is polynomial in $\sizeof{x}$.}
...
There are numerous definitions of succinctness adopted for proof systems in the literature. In this work, we require succinct proof systems only to have sublinear proofs and ``work-saving'' (sublinear) verification\footnote{\url{https://a16zcrypto.com/posts/article/17-misconceptions-about-snarks/\#section--3}}:

\begin{definition}[succinct]
    We say a proof system $\Pi$ is \emph{succinct} if $\sizeof{\pi} \in \bigOmega{\sizeof{C},\allowbreak \sizeof{\witness}}$ and $\Pi.\vrfy(\crs, x, \pi)$ runs in time $\bigOmega{\sizeof{C}}$.
\end{definition}
